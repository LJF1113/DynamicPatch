...
def plot_segmentation_images(
    ...
    # '''
        ...
        savename = str(round(anomaly_score,3))+savename
        savename = os.path.join(savefolder, savename)
        # print(sum(sum(sum(mask))))
        # if sum(sum(sum(mask)))==0 or anomaly_score<0.5 :
        #     continue
        # cv2.imwrite(savename[:-4]+'_img.jpg',image.transpose(1, 2, 0))
        # cv2.imwrite(savename[:-4]+'_segmentation.jpg',mask.transpose(1, 2, 0))
        figure, axes = plt.subplots(1, 2 + int(masks_provided))
        axes[0].imshow(image.transpose(1, 2, 0))
        # axes[2].imshow(segmentation)
        # print(segmentation.shape)
        axes[1].imshow(image.transpose(1, 2, 0), alpha=0.8)
        axes[1].imshow(segmentation, alpha=0.8,cmap='jet')
