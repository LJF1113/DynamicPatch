...
import src.dynamicpatch as dynamicpatch
import src.dynamicpatch_base as dynamicpatch_base
import src.dynamicpatch_online as dynamicpatch_online
import src.dynamicpatch_online_base as dynamicpatch_online_base
...
def parse_args():
    # project
    parser = argparse.ArgumentParser(description='DynamicPatch')
    ...
    # DynamicPatch hyper-parameter
    parser.add_argument("--weight_method", type=str, default="lof_gpu")
    parser.add_argument("--threshold", type=float, default=0.3)
    parser.add_argument("--lof_k", type=int, default=6)
    parser.add_argument("--without_dynamic_weight", action='store_true')
    parser.add_argument("--bank_size", type=float)
    # dataset
    ...
    parser.add_argument("--norm_data", type=bool, default=False)
...
def get_dataloaders(args):
    ...
    for subdataset in args.subdatasets:
        if "mvtec" in args.dataset or "btad" in args.dataset:
        ...
        if "real_iad" in args.dataset:
            json_path = args.exp_json_path
            train_dataset = dataset_library.__dict__[dataset_info[1]](
                source=data_path,
                json_path=json_path,
                classname=subdataset,
...
def get_coreset(args, imagesize, sampler, device):
    ...
    for backbone_name, layers_to_extract_from in zip(
        ...
        if args.bank_size is not None:
            if args.norm_data:
                coreset_instance = dynamicpatch_online_base.DynamicPatch_Online_Base(device)
            else:
                coreset_instance = dynamicpatch_online.DynamicPatch_Online(device)
            ...
            dynamic_weight_flag=not args.without_dynamic_weight,
            ...
            if args.norm_data:
                coreset_instance = dynamicpatch_base.DynamicPatch_Base(device)
            else:
                coreset_instance = dynamicpatch.DynamicPatch(device)
            coreset_instance.load(
                backbone=backbone,
                layers_to_extract_from=layers_to_extract_from,
                device=device,
...
def run(args):
    ...
    # Prepare dataset (include nosiy data)
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        # image_paths = [
        #             x[2] for x in
        #             dataloaders["testing"].dataset.data_to_iterate
        #         ]
        # print(image_paths)
        LOGGER.info(
        ...
        with device_context:
            ...
            if args.bank_size is not None:
                sampler = get_sampler(args.sampler_name, 0.5, device)
            else:
            ...
            # visual or metrics
            segmentations = (segmentations - min_seg_scores) / (max_seg_scores - min_seg_scores)
            ...
            if args.save_segmentation_images and args.dataset!='real_iad':
            ...
            if isinstance(masks_gt, list):
                masks_gt = np.stack(masks_gt)
                
            # print(masks_gt.shape)
            # print(segmentations.shape)
            masks_gt = masks_gt[:,0,:,:]
            # masks_gt = np.where(masks_gt>0,1,0)
            results = metrics.compute_adeval_pro(masks_gt,segmentations,scores,min_scores,max_scores,min_seg_scores,max_seg_scores)
            auroc = results['i_auroc']
            full_pixel_auroc = results['p_auroc']
            aupro = results['p_aupro']
